// Pure view-model for the Harness Flow surface. All rendering decisions for
// the /api/harness-flow envelope live here so Node tests can prove every UI
// state (fresh, stale, malformed, unavailable, INACCESSIBLE, USER_REPORTED,
// and component drill-downs) without a browser. The JSX view only maps this
// model onto elements.
...
function coverageViewModel(coverage) {
  ...
  const limits = Object.entries(coverage.limits || {}).map(([key, value]) => ({ key, value: String(value) }));
  const inspectedParts = [];
  if (Number.isFinite(coverage.inspectedEntries)) inspectedParts.push(`${coverage.inspectedEntries} entries`);
  if (Number.isFinite(coverage.inspectedBytes)) inspectedParts.push(formatBytes(coverage.inspectedBytes));
  return {
    limits,
    inspectedLabel: inspectedParts.length ? inspectedParts.join(" · ") : "inspection counts unavailable",
...
// One scope report (root or component) -> everything its panel renders.
...
// Full /api/harness-flow envelope -> the view's top-level model.
export function buildHarnessFlowViewModel(envelope) {
  ...
  const presentation = STATUS_PRESENTATION[envelope.status] || { label: "Unavailable", tone: "bad" };
  return {
    ...
    ageLabel: Number.isFinite(envelope.ageMinutes) ? formatAge(envelope.ageMinutes) : "age unknown",
