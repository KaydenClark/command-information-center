...
export function PortfolioProjectsView() {
  const state = usePortfolio();
  return <div className="foundry-page"><Header icon={Boxes} eyebrow="Enrollment" title="Projects" detail="Every active producer/module lane, including GPT_OS, with stable identity and its current control coverage." meta="Registry-backed" busy={state.busy} onReload={state.reload} /><PortfolioState {...state} />{state.payload ? <div className="project-portfolio-grid">{state.payload.scopes.map((scope) => <article className="project-portfolio-card" key={scope.id}><div><span className="project-mark">{scope.projectId}</span><span className={cx("status-label", scope.next?.status === "ok" ? "ok" : "bad")}>{scope.next?.status}</span></div><h3>{scope.name}</h3><a href={`https://github.com/${scope.remote}`} target="_blank" rel="noreferrer">{scope.remote}<ExternalLink size={13} /></a><p>{scope.notes}</p><dl><div><dt>Stable specs</dt><dd>{scope.board?.specCount ?? "unavailable"}</dd></div><div><dt>Branch</dt><dd>{scope.deployment?.currentBranch || "unavailable"}</dd></div></dl></article>)}</div> : null}</div>;
}

export function PortfolioDeploymentsView() {
  const state = usePortfolio();
  return <div className="foundry-page"><Header icon={Cloud} eyebrow="Git and release evidence" title="Deployments" detail="Declared remote, producer checkout, installed product, runtime, exact SHA, dirtiness, and evidence boundary for every enrolled scope." meta="No fetch performed" busy={state.busy} onReload={state.reload} /><PortfolioState {...state} />{state.payload ? <section className="deployment-ledger"><div className="deployment-row deployment-head"><span>Project</span><span>Remote</span><span>Observed branches</span><span>Producer HEAD</span><span>State</span></div>{state.payload.scopes.map((scope) => <article className="deployment-row" key={scope.id}><div><span className="project-mark">{scope.projectId}</span><strong>{scope.name}</strong></div><a href={`https://github.com/${scope.remote}`} target="_blank" rel="noreferrer">{scope.remote}</a><div className="deployment-branches"><code><GitBranch size={13} />producer · {scope.deployment?.currentBranch || "unavailable"}</code>{scope.deployment?.installed ? <code><GitBranch size={13} />installed · {scope.deployment.installed.currentBranch || "unavailable"}</code> : null}{scope.deployment?.runtime ? <code><CircleDot size={13} />runtime · {scope.deployment.runtime.currentBranch || "unavailable"}</code> : null}</div><code>{scope.deployment?.headSha?.slice(0, 12) || "—"}</code><div><span className={cx("status-label", scope.deployment?.status === "observed" ? "ok" : "bad")}>{scope.deployment?.status}</span><small>{scope.deployment?.dirtyFiles == null ? "dirty unknown" : `${scope.deployment.dirtyFiles} changed`} · {scope.deployment?.evidence}</small></div><p>{scope.deployment?.detail} {scope.deployment?.upstream ? `Upstream ${scope.deployment.upstream}: ${scope.deployment.aheadBy} ahead / ${scope.deployment.behindBy} behind.` : "No upstream is configured for this checkout."}{scope.deployment?.installed ? ` Installed ${scope.deployment.installed.headSha?.slice(0, 12) || "unavailable"}; ${scope.deployment.installed.dirtyFiles ?? "unknown"} changed.` : ""}{scope.deployment?.runtime ? ` Runtime PID ${scope.deployment.runtime.pid} is served from the installed product.` : ""}</p></article>)}</section> : null}</div>;
}
