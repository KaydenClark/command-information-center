...
// Derived read-only Harness Flow view (S-023). Everything shown here comes
// from the sanitized Audit Engine export envelope; this view issues no write
// requests and offers no audit, repair, or resolution control.
...
function EvidenceBadge({ state, tone }) {
  return <span className={cx("harness-evidence", `harness-evidence-${tone}`)}>{state}</span>;
}

function StageRail({ stages }) {
  return (
    <ol className="harness-stage-rail">
      {stages.map((stage, index) => (
        <li key={stage.key} className={cx("harness-stage", `harness-stage-${stage.tone}`)}>
          ...
          {index < stages.length - 1 ? <ChevronRight size={14} className="harness-stage-arrow" /> : null}
        </li>
      ))}
    </ol>
  );
  ...
  return (
    <div className="harness-run-evidence">
      <div className="harness-evidence-card">
        ...
        <span className={cx("harness-evidence", `harness-evidence-${report.receipt.tone}`)}>{report.receipt.status}</span>
  ...
  return (
    <div className="harness-controls">
      <p className="harness-note">
        <ShieldAlert size={14} />
        Setup controls are static availability only. A listed control is not proof it shaped a run;
        only the receipt-proven stages above carry run evidence.
      </p>
      <table className="harness-controls-table">
        <thead>
          <tr><th>Control</th><th>Kind</th><th>Size</th><th>Digest</th><th>Runtime evidence</th></tr>
        </thead>
        <tbody>
          {controls.map((control, index) => (
            <tr key={`${control.name}-${index}`}>
              ...
              <td><EvidenceBadge state={control.runtime} tone={control.runtimeTone} /></td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="harness-coverage-summary">
        ...
        {coverage.limitReached ? (
          <span className="harness-evidence harness-evidence-warn">
            <AlertTriangle size={12} /> limit reached — coverage is partial
          </span>
        ) : null}
      </div>
      {coverage.limits.length ? (
        <div className="harness-coverage-limits">
          {coverage.limits.map((limit) => (
            <span key={limit.key}><strong>{limit.key}</strong> {limit.value}</span>
          ))}
        </div>
      ) : null}
  ...
  return (
    <div className="panel harness-panel">
      <div className="panel-title">
        ...
        <button className="status-button" onClick={load} disabled={busy} aria-label="Reload harness flow">
          <RefreshCw size={15} className={busy ? "spin" : ""} />
          <span>Reload</span>
        </button>
      </div>

      <div className={cx("harness-banner", `harness-banner-${vm.tone}`)} data-status={vm.status}>
      ...
      {vm.root ? (
        <section className="harness-root">
          <div className="section-header">
            ...
            <small>Available → Eligible → Shown → Consulted → Acted through → Checked → Accepted</small>
          </div>
            ...
            <div className="harness-component-list">
              {vm.components.map((component) => (
                <button
                  ...
                  onClick={() => setSelectedComponent(selectedComponent === component.key ? null : component.key)}
                >
                  <FileCheck2 size={14} />
          ...
          {selected ? (
            <div className="harness-drilldown">
              <div className="harness-drilldown-head">
                <button className="icon-button" onClick={() => setSelectedComponent(null)} aria-label="Close component drill-down">
                  <ChevronLeft size={15} />
                </button>
              ...
              {selected.report ? (
                ...
                <p className="harness-malformed">
                  This component report is {selected.status}: {selected.reason || "no readable evidence"}
                </p>
              )}
            </div>
          ) : null}
