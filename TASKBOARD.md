...
| T-008 | 2 | Reconcile Intelligence routes, docs, and tests | Audit gap 4; UI uses `/api/intelligence/kb` while docs emphasize `/overview`, `/ask`, and `/sources` | `server/intelligence.js`, `src/intelligence.jsx`, `README.md`, `BLUEPRINT.md`, `RUNBOOK.md`, tests | Red/green route contract tests; manual missing-credential checks; `npm test`; `npm run build` | Update `README.md`, `BLUEPRINT.md`, `RUNBOOK.md`, `TASKBOARD.md` | agent | ready | 2026-07-07 |
| T-002 | 2 | Deepen the CIC task board toward ClickUp-style board/list workflows | Audit gap 5 accepted; recent CIC direction calls for richer cards, grouped list/swimlane views, and smoother task operations | `src/main.jsx`, `src/styles.css`, `server/db.js`, `test/*.test.js` | Red/green tests for task behavior, `npm test`, `npm run build`, desktop and mobile browser checks | Update `BLUEPRINT.md`, `TASKBOARD.md`, `README.md`, and wiki if behavior changes | agent | ready | 2026-07-07 |
