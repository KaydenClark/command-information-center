...
| [S-022](specs/S-022-skill-catalog-visibility/SPEC.md) | TK-003: Richer drift and fail-closed states: classify drifted / missing / deployed-only and render fail-closed stale/unavailable states, with fixtures and desktop+mobile proof per state (ready) | CIC Engineer; Kayden (catalog acceptance) | none | 2026-07-24: TK-002 closed — the Skills view now widens to every catalog entry in catalog order, each with name, definition, rewrite lane, availability, per-entry provenance (source file), and freshness. Live runtime renders all 33 real Forge catalog entries; new multi-row/order fixtures cover the model, React SSR, route, and desktop+mobile browser layers without changing the TK-001 route or payload contract. | TK-003 — richer drift taxonomy (drifted / missing / deployed-only) and per-state fail-closed stale/unavailable states with per-state desktop+mobile proof. |
<!-- hot-specs:end -->
