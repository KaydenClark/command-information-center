...
| GET | `/api/harness-flow` | passcode when configured | Return the derived read-only Harness Flow envelope from the injected sanitized Audit Engine export reader; no write, audit, repair, or resolution route exists | `server/harnessFlow.js` |
...
- The Harness Flow surface is a derived consumer of the sanitized Audit Engine
  export behind an injected reader seam. Freshness comes from the report's own
  generation time; missing or unrecognized run evidence stays visibly
  `INACCESSIBLE`; static setup controls are never promoted into receipt-proven
  stages; and the envelope exposes no absolute paths, full digests, or raw
  report internals. CIC cannot run an audit, apply a repair, or resolve a
  finding through this surface.
