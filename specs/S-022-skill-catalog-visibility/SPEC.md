...
**Blockers:** none
**Latest event:** 2026-07-24: TK-002 closed — the Skills view now widens to every catalog entry in catalog order, each with name, definition, rewrite lane, availability, per-entry provenance (source file), and freshness. Live runtime renders all 33 real Forge catalog entries; new multi-row/order fixtures cover the model, React SSR, route, and desktop+mobile browser layers without changing the TK-001 route or payload contract.
**Next gate:** TK-003 — richer drift taxonomy (drifted / missing / deployed-only) and per-state fail-closed stale/unavailable states with per-state desktop+mobile proof.
...
| TK-002 | Widen coverage: parse and render every catalog entry in catalog order with per-entry name, definition, lane, availability, provenance, and freshness | done | none | Full-catalog render in catalog order with per-entry provenance/freshness; shared multi-row fixture drives model, React SSR, route, and desktop+mobile browser proof; live runtime renders all 33 real Forge entries — see Evidence 2026-07-24 |
| TK-003 | Richer drift and fail-closed states: classify drifted / missing / deployed-only and render fail-closed stale/unavailable states, with fixtures and desktop+mobile proof per state | ready | none | pending |
...
- [x] The Skills view lists every catalog entry with name, definition, lane,
      and availability in catalog order. (TK-002)
- [x] Every entry shows its source and freshness; nothing renders as fresher
      than its source. (TK-002)
...
| 2026-07-24 | TK-002 | Widened the Skills view from the one-row tracer to the full catalog. The already-full-catalog server reader (`server/skillCatalog.js`) and `GET /api/skills` route/payload contract are unchanged; the widening was at the view/model/test layers. `src/skillCatalogModel.js` now exposes a per-entry `provenanceLabel` (the entry's own source file — `skills/README.md` for catalog rows, `.claude/skills/` for deployed-only). `src/skills.jsx` renders a new per-entry Provenance field alongside availability, freshness, and the canon-vs-deployed badge, for every entry in catalog order. `src/styles.css` moves `.skill-entry-fields` to an `auto-fit` grid so the added column reflows on desktop and collapses on mobile, with `.skill-provenance` overflow-safe. New shared deterministic fixture `test/fixtures/skillCatalogPayload.js` (non-alphabetical catalog order + every drift class + both provenance sources) drives multi-row/order coverage across layers. Read-only preserved: no write/sync/deploy route added. | pass — RED first: `node --test test/skillCatalogModel.test.js` failed on missing `provenanceLabel` (actual `undefined`); GREEN after model+view change. `node --test test/skillCatalogModel.test.js` 6/6 (adds full-catalog order + per-entry provenance + distinct per-entry freshness). `node --test test/skillsReact.test.js` 5/5 (adds catalog-order SSR render + per-entry provenance render). `node --test test/skillsRoute.test.js` 4/4 (adds 5-entry ordered route proof: in_sync/drifted/missing/pending/deployed_only + provenance + counts). Full `npm test` 316 tests / 310 pass / 6 pre-existing todo / 0 fail (+5 new vs TK-001's 311/305). `npm run test:browser` 28 pass / 8 expected skip (baseline 26 + 2 new widened specs across the desktop and iPhone 13 projects: full-catalog count, exact catalog order via rendered `<strong>` names, per-entry provenance count + both sources visible, full drift taxonomy badges, and zero horizontal overflow with the extra column). `npm run build` clean Vite production build. `npm audit --omit=dev` unchanged: one pre-existing low-severity `body-parser` advisory (GHSA-v422-hmwv-36x6), no dependency manifest touched. `node tools/spec-workbench.mjs doctor` clean. Live demo: built server against the real config renders `GET /api/skills` with `status: ok, total: 33, active: 33` in Forge catalog order with per-entry provenance; desktop and mobile Skills-view screenshots recorded at `specs/S-022-skill-catalog-visibility/evidence/s022-tk002-skills-desktop.png` and `…-mobile.png`. Remaining gap: TK-003 (richer drift taxonomy + fail-closed per-state proof) is now unblocked and ready. |
